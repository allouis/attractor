...
| # | Deliverable | Status |
|---|---|---|
| T9a | **Finished-run state survives a daemon restart.** Graph node status is painted from the daemon's *in-memory* event history (`registry.history`), replayed over SSE on open. After a daemon restart that memory is gone, so an old completed run repaints nothing → nodes stale as "executing". Persist/replay from the on-disk `events.jsonl`: when a run isn't in memory (or always), reconstruct its history from `events.jsonl` on Subscribe so the graph/timeline/terminal node states are correct. Acceptance: complete a run, restart the daemon, open it → every node terminal (completed/failed), nothing "executing"; timeline intact. Keep live runs unchanged. | done |
