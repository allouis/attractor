...
|---|---|---|---|
| W1 | Design foundation + nav shell: token system (surface/text ramp, spacing/type scale, state palette), dark/light with `prefers-color-scheme` + persisted toggle; restyle the existing Runs list + Run detail to tokens; 3-tab nav (Items/Runs/Workflows) + hash routing, default Items | — | todo |
| W2 | `GET /workflows` catalog + `GET /workflows/{name}/graph` (SVG via `render.SVG`), server-side, with tests | — | todo |
