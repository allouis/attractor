...
| # | Deliverable | Status |
|---|---|---|
| T9a | **Finished-run state survives a daemon restart.** Graph node status is painted from the daemon's *in-memory* event history (`registry.history`), replayed over SSE on open. After a daemon restart that memory is gone, so an old completed run repaints nothing → nodes stale as "executing". Persist/replay from the on-disk `events.jsonl`: when a run isn't in memory (or always), reconstruct its history from `events.jsonl` on Subscribe so the graph/timeline/terminal node states are correct. Acceptance: complete a run, restart the daemon, open it → every node terminal (completed/failed), nothing "executing"; timeline intact. Keep live runs unchanged. **REOPENED — the first pass (`reconstructHistory`/`replayEvents`/`resolveInterrupted`) works for single-visit nodes but DROPS the re-entry history of looped nodes: for run `8f33e2e1c460a78c`, on disk `review_loop` has `started(seq397)→failed(795)→started(1091)→completed(1207)` but the reconstructed stream serves ONLY `started(397)`, so a `stack.manager_loop` / looped node (also `implement`/`fix`/`verify` in a fix loop) stays "running". Fix: reconstruction must replay ALL of a node's events in seq order (every visit), so a re-entered node reaches its true final terminal. Add a regression test over an events.jsonl with a node visited twice → final state completed. Re-verify on `8f33e2e1c460a78c`: `review_loop` must NOT be running.** | done |
| T9b | **Drop the dedicated "Review notes" section — attribute nested findings to their node.** "Review notes" bakes `build-review` semantics into the generic view. A `stack.manager_loop` node's forwarded child-pipeline verdict/findings are really *that node's* output — surface them in the **node inspector** (click the node → see its child's verdict/findings), and remove the special top-level Review-notes section + the `classifyPipelineFailed` "review" case. The run's OWN failure stays the red banner. Acceptance: a `build-review` run shows its review findings under the `review_loop` node's inspector; no dedicated section; a workflow with no child pipeline shows no empty leftover. | done |
| T9c | **Derive the run's produced diff from jj (not an uploaded artifact).** web-ui-v2 U4 wants "the run's produced change", but the panel only reads a `*.diff` artifact, which self-dev pipelines never upload → always empty. Record the run's jj revision range — the repo `@` change-id at run start (base) and at end (tip) — on the run, and serve `jj diff --from <base> --to <tip>` for the diff panel; keep the artifact path as a fallback. Applies to runs whose cwd is a host jj repo (direct runner); VM runs stay "no diff" until results-export (vm-workspace G2). Acceptance: a `build-review` run's Diff panel shows the milestone's real change. | done |
