...
| C8 | Docs: fix attractor-spec §4.5 (runtime context interpolation; drop the misleading `$goal`-only prose; note the `tool_command` deviation); update router-spec/items-spec (R3 reframe, `$context.` syntax); this spec's deviations. | C7 | done |
