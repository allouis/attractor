Amend the existing pull request — #$context.pr_number in $context.repo —
to address the reviewer feedback. The PR is checked out in your working
directory; its commits are your working copy's ancestors. You **extend
this branch**, you do not start over.
...
Feedback the human left when approving the plan (empty if they approved
without a note — treat any text here as an instruction to honour while
amending):
...
Read the relevant existing code and the PR's own change
(`jj diff --from 'trunk()' --to @`) before you touch anything — prefer
extending the branch's existing patterns over inventing new ones. Then work
one vertical slice at a time, red/green:
...
For a bug the feedback reports, the first slice reproduces it: a test that
fails on the current code because of the bug, then the fix that turns it
green.
...
followed by an adversarial multi-lens review of the whole PR. Anything
that fails comes back to you here as a focused follow-up; you keep this
context.

Stay inside the slice. Spot an unrelated problem? Note it in your
response; do not fix it here. Do NOT push or move bookmarks — publishing
happens after the ship gate. Run the repo's formatter before you finish.
...
```json
{ "outcome": "fail", "failure_reason": "a feedback point needs a design decision the plan didn't resolve" }
```
