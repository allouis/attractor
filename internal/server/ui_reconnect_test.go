...
// the server replays the full history again, and the UI must not re-append the
// per-node prose chunks it already applied. Drives the real attachStream from
// the embedded index.html via node's vm sandbox with a fake EventSource.
...
// Expose the lexically-scoped nodeProse so the test can inspect it.
...
const log = sandbox.__nodeProse['n1'] || [];
...
`
	...
	if got != "2" {
		t.Errorf("node-prose chunk count = %s, want 2 (reconnect duplicated chunks)", got)
	}
}
