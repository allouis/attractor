...
// node, re-executing the one that failed. A non-failed run has nothing to
// resume (409); an unknown run is a 404. Cancel and fresh re-run are separate:
// cancel already shipped, and a fresh re-run resubmits via POST
// /workflows/{name}/run.
func (s *Server) restartPipeline(w http.ResponseWriter, r *http.Request) {
	...
	if !run.prepareRestart() {
		http.Error(w, "run is not failed; nothing to resume", http.StatusConflict)
		return
	}
